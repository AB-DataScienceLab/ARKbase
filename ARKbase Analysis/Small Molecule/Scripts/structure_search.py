...
def search_and_generate(query_smiles, threshold, db_path): # No longer needs image_path
    fpgen = rdFingerprintGenerator.GetMorganGenerator(radius=2, fpSize=1024)

    query_mol = Chem.MolFromSmiles(query_smiles.strip())
    if query_mol is None:
        print("â�Œ Error: Query SMILES could not be parsed.", file=sys.stderr)
        return []
        ...
        with open(db_path, mode='r', encoding='utf-8-sig') as csvfile:
            ...
            for row in reader:
                ...
                if similarity >= threshold:
                    # --- NEW: Generate Base64 image for this single molecule ---
                    ...
                    found_molecules.append({
                        ...
                        "image_base64": base64_img  # Add image data to JSON
                    })
    
    except Exception as e:
        print(f"â�Œ Error reading the database file: {e}", file=sys.stderr)
        return []

    if not found_molecules:
        print("âš ï¸� No structures found above the threshold.", file=sys.stderr)
