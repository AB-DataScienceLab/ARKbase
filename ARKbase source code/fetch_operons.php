...
// Get the pathogen name from the GET request (e.g., ?pathogen=a_baumannii)
...
try {
    ...
    // --- SQL QUERY FOR THE operon_AD TABLE ---
    // Select all required columns from the operon_AD table
    $sql = "SELECT
                Locus_Tag,
                Operon_ID,
                Operon_Length,
                ...
                Product
            FROM operon_AD
            WHERE Pathogen = ?"; // Filter by the 'Pathogen' column using a placeholder
    ...
    if ($result) {
        ...
        while($row = $result->fetch_assoc()) {
            // Trim whitespace from string values for consistency
