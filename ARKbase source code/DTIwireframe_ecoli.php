   ...
   function renderTable() {
    ...
    if (dataToRender.length === 0) {
        ...
        dataToRender.forEach(item => {
            ...
            // --- CORRECTED LOGIC FOR TOXICITY ---
            let toxicityHtml = 'N/A'; // Default value
            // Check if the property exists and is not null/empty
            if (item.DeepPK_toxicity_Safe && item.DeepPK_toxicity_Safe.trim() !== '') {
                
                // Check if the value is 'yes' (case-insensitive)
                const isSafe = item.DeepPK_toxicity_Safe.toLowerCase() === 'yes';
                
                // Determine badge color based on the result
                const badgeClass = isSafe ? 'bg-success' : 'bg-danger';
                
                // Create the HTML for the badge
                toxicityHtml = `<span class="badge ${badgeClass}">${item.DeepPK_toxicity_Safe}</span>`;
            }
            ...
            row.innerHTML = `
                ...
                <td>${toxicityHtml}</td> <!-- Use the generated HTML -->
            `;
    ...
    filterNetworkFromCheckboxes();
        ...
        function downloadData() {
            ...
            const headers = [
                ...
                'Gene.Ontology.(cellular.component)', 'drug_image_url', 'DeepPK_toxicity_Safe' // Include all relevant fields
            ];
